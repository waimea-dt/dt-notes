...
- 2020: AlphaFold
	...
	**DeepMind**'s **AlphaFold** demonstrated that neural networks could predict the **three-dimensional structure of proteins** from their amino-acid sequences with remarkable accuracy, solving the 50-year-old '**protein folding problem**', and earning its co-creators the **2024 Nobel Prize in Chemistry**. Historically, scientific computing meant running massive simulations on supercomputers (e.g., weather modelling), but AlphaFold showed that using **machine learning models trained on vast empirical data sets** could bypass simulations entirely.
